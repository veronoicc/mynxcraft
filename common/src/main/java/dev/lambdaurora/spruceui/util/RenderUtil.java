...
public final class RenderUtil {
	...
	public static void renderSelectionBox(int x, int y, int width, int height, int red, int green, int blue, int alpha) {
		...
		bufferBuilder.vertex(x, top, 0).endVertex();
		bufferBuilder.vertex(right, top, 0).endVertex();
		bufferBuilder.vertex(right, y, 0).endVertex();
		bufferBuilder.vertex(x, y, 0).endVertex();
		tessellator.end();
		RenderSystem.setShaderColor(0, 0, 0, 1.f);
		bufferBuilder.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION);
		bufferBuilder.vertex(x + 1, top - 1, 0).endVertex();
		bufferBuilder.vertex(right - 1, top - 1, 0).endVertex();
		bufferBuilder.vertex(right - 1, y + 1, 0).endVertex();
		bufferBuilder.vertex(x + 1, y + 1, 0).endVertex();
		tessellator.end();
	}
}
