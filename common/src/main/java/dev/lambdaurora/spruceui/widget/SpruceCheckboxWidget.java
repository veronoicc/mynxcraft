...
public class SpruceCheckboxWidget extends AbstractSpruceBooleanButtonWidget {
	...
	protected void renderButton(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
		...
		if (this.getValue()) {
			...
			graphics.blit(TEXTURE, this.getX(), this.getY(), 0.f, 40.f, this.getHeight(), this.getHeight(), 64, 64);
		} else if (this.showCross) {
			if (this.colored)
				RenderSystem.setShaderColor(1.f, 0.f, 0.f, this.alpha);
			graphics.blit(TEXTURE, this.getX(), this.getY(), 0.f, 20.f, this.getHeight(), this.getHeight(), 64, 64);
		}
		...
		if (this.showMessage) {
			FormattedCharSequence message = Language.getInstance().getVisualOrder(this.client.font.substrByWidth(this.getMessage(), this.getWidth() - this.getHeight() - 4));
			graphics.drawString(Minecraft.getInstance().font, message, this.getX() + this.getHeight() + 4, this.getY() + (this.getHeight() - 8) / 2,
	...
	protected void renderBackground(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
		...
		RenderSystem.defaultBlendFunc();
		RenderSystem.blendFunc(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA);
		graphics.blit(TEXTURE, this.getX(), this.getY(), this.isFocusedOrHovered() ? 20.f : 0.f, 0.f, this.getHeight(), this.getHeight(), 64, 64);
	}
