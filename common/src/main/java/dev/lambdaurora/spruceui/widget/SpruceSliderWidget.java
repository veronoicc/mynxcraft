...
public class SpruceSliderWidget extends AbstractSpruceButtonWidget implements Tooltipable {
	...
	protected void renderButton(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
		...
		final ResourceLocation texture = this.isFocusedOrHovered() ? SLIDER_HANDLE_HIGHLIGHTED : SLIDER_HANDLE;
		graphics.blitSprite(texture, this.getX() + (int) (this.value * (double) (this.getWidth() - 8)), this.getY(), 8, 20);
