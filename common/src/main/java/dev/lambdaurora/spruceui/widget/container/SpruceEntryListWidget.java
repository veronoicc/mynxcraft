...
public abstract class SpruceEntryListWidget<E extends SpruceEntryListWidget.Entry> extends AbstractSpruceParentWidget<E>
	...
	protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
		...
		if (this.shouldRenderTransition()) {
			...
			// TOP
			buffer.vertex(left, top + 4, 0).color(0, 0, 0, 0).endVertex();
			buffer.vertex(right, top + 4, 0).color(0, 0, 0, 0).endVertex();
			buffer.vertex(right, top, 0).color(0, 0, 0, 255).endVertex();
			buffer.vertex(left, top, 0).color(0, 0, 0, 255).endVertex();
			// RIGHT
			buffer.vertex(right - 4, bottom, 0).color(0, 0, 0, 0).endVertex();
			buffer.vertex(right, bottom, 0).color(0, 0, 0, 255).endVertex();
			buffer.vertex(right, top, 0).color(0, 0, 0, 255).endVertex();
			buffer.vertex(right - 4, top, 0).color(0, 0, 0, 0).endVertex();
			// BOTTOM
			buffer.vertex(left, bottom, 0).color(0, 0, 0, 255).endVertex();
			buffer.vertex(right, bottom, 0).color(0, 0, 0, 255).endVertex();
			buffer.vertex(right, bottom - 4, 0).color(0, 0, 0, 0).endVertex();
			buffer.vertex(left, bottom - 4, 0).color(0, 0, 0, 0).endVertex();
			// LEFT
			buffer.vertex(left, bottom, 0).color(0, 0, 0, 255).endVertex();
			buffer.vertex(left + 4, bottom, 0).color(0, 0, 0, 0).endVertex();
			buffer.vertex(left + 4, top, 0).color(0, 0, 0, 0).endVertex();
			buffer.vertex(left, top, 0).color(0, 0, 0, 255).endVertex();
			tessellator.end();
		}
		...
		if (maxScroll > 0) {
			...
			scrollbarHeight = Mth.clamp(scrollbarHeight, 32, this.getHeight() - 8);
	...
	protected void renderScrollbar(Tesselator tessellator, BufferBuilder buffer, int scrollbarX, int scrollbarEndX, int scrollbarY, int scrollbarHeight) {
		RenderSystem.setShader(GameRenderer::getPositionColorShader);
		buffer.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
		buffer.vertex(scrollbarX, this.getY() + this.getHeight(), 0.0).color(0, 0, 0, 255).endVertex();
		buffer.vertex(scrollbarEndX, this.getY() + this.getHeight(), 0.0).color(0, 0, 0, 255).endVertex();
		buffer.vertex(scrollbarEndX, this.getY(), 0.0).color(0, 0, 0, 255).endVertex();
		buffer.vertex(scrollbarX, this.getY(), 0.0).color(0, 0, 0, 255).endVertex();
		buffer.vertex(scrollbarX, scrollbarY + scrollbarHeight, 0.0).color(128, 128, 128, 255).endVertex();
		buffer.vertex(scrollbarEndX, scrollbarY + scrollbarHeight, 0.0).color(128, 128, 128, 255).endVertex();
		buffer.vertex(scrollbarEndX, scrollbarY, 0.0).color(128, 128, 128, 255).endVertex();
		buffer.vertex(scrollbarX, scrollbarY, 0.0).color(128, 128, 128, 255).endVertex();
		buffer.vertex(scrollbarX, scrollbarY + scrollbarHeight - 1, 0.0).color(192, 192, 192, 255).endVertex();
		buffer.vertex(scrollbarEndX - 1, scrollbarY + scrollbarHeight - 1, 0.0).color(192, 192, 192, 255).endVertex();
		buffer.vertex(scrollbarEndX - 1, scrollbarY, 0.0).color(192, 192, 192, 255).endVertex();
		buffer.vertex(scrollbarX, scrollbarY, 0.0).color(192, 192, 192, 255).endVertex();
		tessellator.end();
	}
