...
public class SpruceLabelWidget extends AbstractSpruceWidget implements Tooltipable, WithBorder {
	...
	protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
		...
		for (var it = this.lines.iterator(); it.hasNext(); y += 9) {
			var line = it.next();
			int x = this.centered ? (this.getInnerX() + this.maxWidth / 2) - this.client.font.width(line) / 2
					: this.getInnerX();
			graphics.drawString(this.client.font, line, x, y, 10526880, true);
		}
		...
		if (this.tooltip != null) {
			if (!this.tooltip.getString().isEmpty()) {
				var wrappedTooltipText = this.client.font.split(this.tooltip, Math.max(this.width / 2, 200));
				if (this.hovered)
