...
public class SpruceTextAreaWidget extends AbstractSpruceTextInputWidget {
	...
	public SpruceTextAreaWidget(Position position, int width, int height, Component title) {
		...
		this.displayedLines = this.getInnerHeight() / this.textRenderer.lineHeight;
	...
	protected boolean doesLineOccupyFullSpace(String cursorLine) {
		return this.textRenderer.width(cursorLine) >= this.getInnerWidth();
	}
	...
	protected boolean onMouseClick(double mouseX, double mouseY, int button) {
		if (button == 0) {
			int x = Mth.floor(mouseX) - this.getX() - 4;
			int y = Mth.floor(mouseY) - this.getY() - 4;
			...
			this.onSelectionUpdate(() -> {
				...
				this.cursor.lastColumn = this.cursor.column = this.textRenderer.plainSubstrByWidth(this.lines.get(row), x).length();
			});
	...
	protected void drawText(GuiGraphics graphics) {
		...
		for (int row = this.firstLine; row < this.firstLine + length; row++) {
			...
			graphics.drawString(this.textRenderer, Component.literal(line), textX, lineY, textColor);
			this.drawSelection(graphics, line, lineY, row);

			lineY += this.textRenderer.lineHeight;
		}
	}
	...
	protected void drawSelection(GuiGraphics graphics, String line, int lineY, int row) {
		...
		int x = this.getX() + 4 + this.textRenderer.width(line.substring(0, startIndex));
		var selected = line.substring(startIndex, endIndex);

		int x2 = x + this.textRenderer.width(selected);
		int y2 = lineY + this.textRenderer.lineHeight;
		...
		buffer.vertex(x, y2, 0.d).endVertex();
		buffer.vertex(x2, y2, 0.d).endVertex();
		buffer.vertex(x2, lineY, 0.d).endVertex();
		buffer.vertex(x, lineY, 0.d).endVertex();
		tessellator.end();
	...
	protected void drawCursor(GuiGraphics graphics) {
		if (!this.isFocused())
			return;
		if (this.lines.isEmpty()) {
			graphics.drawString(this.textRenderer, Component.literal("_"), this.getX(), this.getY() + 4, ColorUtil.TEXT_COLOR);
			return;
		}
		...
		int cursorX = this.getX() + 4 + this.textRenderer.width(cursorLine.substring(0, this.cursor.column));
		int cursorY = this.getY() + 4 + actualRow * this.textRenderer.lineHeight;
		...
		if (this.cursor.row < this.lines.size() - 1 || this.cursor.column < cursorLine.length() || this.doesLineOccupyFullSpace(cursorLine))
			...
			graphics.drawString(this.textRenderer, "_", cursorX, cursorY, ColorUtil.TEXT_COLOR);
	}
