...
public class SpruceTabbedWidget extends AbstractSpruceParentWidget<SpruceWidget> {
	...
	protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
		if (this.title != null) {
			graphics.drawCenteredString(this.client.font, this.title, this.getX() + this.list.getWidth() / 2,
	...
	public static class TabEntry extends Entry {
		...
		protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
			...
			for (var it = this.title.iterator(); it.hasNext(); y += 9) {
				var line = it.next();
				graphics.drawString(this.client.font, line, this.getX() + 4, y, 0xffffff, false);
			}
			if (this.description != null) {
				y += 4;
				for (var it = this.description.iterator(); it.hasNext(); y += 9) {
					var line = it.next();
					graphics.drawString(this.client.font, line, this.getX() + 8, y, 0xffffff, false);
				}
			}
		}
