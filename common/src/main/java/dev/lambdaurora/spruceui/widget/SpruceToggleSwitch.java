...
public class SpruceToggleSwitch extends AbstractSpruceBooleanButtonWidget {
	private static final ResourceLocation TEXTURE = new ResourceLocation("spruceui", "textures/gui/toggle_switch.png");
	...
	protected void renderButton(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
		...
		graphics.blit(TEXTURE, this.getX() + (this.getValue() ? 14 : 0), this.getY() + (this.getHeight() / 2 - 9),
		...
		if (this.showMessage) {
			...
			graphics.drawString(this.client.font, message, this.getX() + 36, this.getY() + (this.getHeight() - 8) / 2,
	...
	protected void renderBackground(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
		...
		RenderSystem.defaultBlendFunc();
		RenderSystem.blendFunc(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA);
		graphics.blit(TEXTURE, this.getX(), this.getY() + (this.getHeight() / 2 - 9),
