...
public class Tooltip implements SprucePositioned {
	...
	public Tooltip(int x, int y, FormattedText tooltip, int parentWidth) {
		this(x, y, Minecraft.getInstance().font.split(tooltip, Math.max(parentWidth * 2 / 3, 200)));
	}
	...
	public void render(GuiGraphics graphics) {
		graphics.renderTooltip(Minecraft.getInstance().font, this.tooltip, DefaultTooltipPositioner.INSTANCE, this.x, this.y);
	}
		...
		if (widget.isVisible()) {
			widget.getTooltip().ifPresent(tooltip -> {
				...
				if (!tooltip.getString().isEmpty() && tooltipTicks >= 45) {
					var wrappedTooltipText = Minecraft.getInstance().font.split(
